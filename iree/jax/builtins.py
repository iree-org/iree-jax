...
class jit_kernel(tracing.CallableIntrinsic):
  ...
  def __init__(self, wrapped_f, *, wrap_with_jit: bool = True):
    ...
    self.jit_f = jax.jit(self.wrapped_f, backend="interpreter") if wrap_with_jit else self.wrapped_f
  ...
  def resolve_call(self, func_trace: tracing.FunctionIrTrace, *args):
    ...
    lowered_asm = lowered.compiler_ir(dialect="stablehlo")
